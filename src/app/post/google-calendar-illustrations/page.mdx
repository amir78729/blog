...
{Object.entries(illustrationsInfo).map(([key, {title, words}]) => (
    <div key={key} >
        ...
        <Img src={`https://ssl.gstatic.com/tmly/f8944938hffheth4ew890ht4i8/flairs/xxhdpi/img_${key}.jpg`} alt={`The image will be shown on your Google Calendar event when you set event title to ${words}.`}/>
    </div>
))}
